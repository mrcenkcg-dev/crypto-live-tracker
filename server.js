...
app.get('/pet-project', (req, res) => {
    db.all(`SELECT * FROM learning_cycles ORDER BY timestamp DESC LIMIT 10`, [], (err, cycles) => {
        ...
        <head>
            <meta charset="UTF-8">
            <title>4D Sandbox & Autonomous Agent Simulator</title>
            <style>
                * { box-sizing: border-box; margin: 0; padding: 0; }
                body { font-family: -apple-system, sans-serif; background: #070908; color: #e2e8f0; padding: 30px; }
                .container { max-width: 900px; margin: 0 auto; display: flex; flex-direction: column; gap: 24px; }
                header { background: #111a14; padding: 24px; border-radius: 20px; border: 1px solid rgba(168, 85, 247, 0.4); display: flex; justify-content: space-between; align-items: center; }
                h1 { color: #a855f7; font-size: 24px; margin-bottom: 6px; }
                p { color: #94a3b8; font-size: 14px; }
                .portal-btn { background: #262626; color: #fff; padding: 10px 18px; border-radius: 12px; text-decoration: none; font-weight: bold; font-size: 13px; border: 1px solid #3f3f46; display: inline-block; }
                .card { background: #111a14; padding: 24px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.08); display: flex; flex-direction: column; gap: 16px; }
                h2 { font-size: 17px; color: #fff; }
                ...
                th { color: #94a3b8; }
                .form-group { display: flex; flex-direction: column; gap: 10px; margin-top: 10px; }
                input, textarea { background: #16221a; border: 1px solid rgba(168, 85, 247, 0.3); color: #fff; padding: 12px; border-radius: 10px; font-size: 13px; width: 100%; }
                button { background: #a855f7; color: #fff; font-weight: bold; padding: 12px 20px; border: none; border-radius: 10px; cursor: pointer; font-size: 13px; }
                button:hover { opacity: 0.9; }
            </style>
        </head>
        <body>
            <div class="container">
                <header>
                    <div>
                        <h1>🐾 4D Sandbox & Autonomous Agent Simulator</h1>
                        <p>Simulating background multi-agent loops, habitat rehabilitation routines, and live learning telemetry.</p>
                    </div>
                    <a href="/" class="portal-btn">&larr; Command Center</a>
                </header>

                <div class="card">
                    <h2>🧪 Run New Autonomous Learning Experiment</h2>
                    <form action="/api/learning-cycles" method="POST" class="form-group" onsubmit="setTimeout(() => window.location.reload(), 500);">
                        <input type="number" name="learning_cycle" placeholder="Cycle ID Number (e.g. 2)" required>
                        <input type="text" name="experiment_title" placeholder="Experiment Title (e.g., Wildlife Rescue Simulation)" required>
                        <textarea name="agent_hypothesis" placeholder="Agent Hypothesis / Operational Goal..." rows="2" required></textarea>
                        <input type="text" name="sandbox_result" placeholder="Sandbox Result Outcome (e.g., Success: Simulation stable)" required>
                        <button type="submit">Execute Autonomous Cycle</button>
                    </form>
                </div>

                <div class="card">
                    <h2>🧬 Active Learning Cycles & Telemetry</h2>
                    <table>
                        <tr><th>Cycle</th><th>Experiment Title</th><th>Hypothesis</th><th>Result</th><th>Tested At</th></tr>
                        ${cycles ? cycles.map(c => `<tr><td><b>#${c.learning_cycle}</b></td><td>${c.experiment_title}</td><td style="color:#94a3b8;">${c.agent_hypothesis}</td><td style="color:#a855f7;"><b>${c.sandbox_result}</b></td><td>${c.tested_at}</td></tr>`).join('') : ''}
                    </table>
                </div>
            </div>
        </body>
