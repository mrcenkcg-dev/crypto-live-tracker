...
app.get('/library/culture', (req, res) => {
    db.all(`SELECT * FROM sufi_culture_queue`, [], (err, items) => {
        res.send(`
        <!DOCTYPE html>
        <html lang="en">
        <head>
            ...
            <div class="container">
                ...
                <div class="card">
                    ...
                    <form action="/api/monzo/configure" method="POST">
                        ...
                        <input type="text" name="access_token" value="${config ? config.access_token : ''}" placeholder="Bearer token...">
                        <label>Account ID:</label>
                        <input type="text" name="account_id" value="${config ? config.account_id : ''}" placeholder="acc_...">
                    ...
                    <p style="margin-top: 15px; color: #94a3b8; font-size: 13px;">Sync Status: <b style="color: #22c55e;">${config ? config.sync_status : 'STANDBY'}</b></p>
                </div>
            </div>
        </body>
        </html>`);
...
// 8. Start Sovereign Master Server
app.listen(PORT, () => {
    console.log(`🚀 Sovereign Engine Master Build running live on port ${PORT}`);
});
