...
// 6.5. Automated Playwright-Style Watchdog Loop (Integrated Service)
            ...
            <head>
                ...
                <style>
                    * { box-sizing: border-box; margin: 0; padding: 0; }
                    ...
                    .stream-card { background: #181818; border: 1px solid #333; border-radius: 16px; padding: 20px; display: flex; flex-direction: column; gap: 12px; margin-bottom: 20px; }
                    .stream-title { font-size: 16px; font-weight: bold; color: #fff; }
                    .stream-desc { font-size: 14px; color: #aaa; line-height: 1.4; }
                    video { width: 100%; border-radius: 12px; max-height: 450px; background: #000; }
                    .footer { text-align: center; color: #666; font-size: 12px; padding: 30px 0; border-top: 1px solid #222; margin-top: 40px; }
                </style>
            </head>
            <body>
                <nav>
                    ...
                    <div class="nav-actions">
                        ...
                        <a href="/pet-project" class="nav-btn" style="background: #a855f7; border-color: #a855f7;">🐾 4D Sandbox</a>
                    </div>
                </nav>
                <div class="container">
                    <div class="hero-card">
                        <h1>Welcome to Anadolu Island</h1>
                        <p>A decentralized community hub, autonomous media feed, and micro-fee digital toll gate network running live telemetry.</p>
                    </div>
                    <div>
                        <div class="feed-header">
                            <span>📹 Active Media & Video Broadcast Streams</span>
                            <span style="font-size: 13px; color: ${accentColor}; font-weight: bold;">Toll Gate: $0.001 Verified</span>
                        </div>
                        ${mediaStreams && mediaStreams.length > 0 ? mediaStreams.map(s => `
                            <div class="stream-card">
                                <div class="stream-title">${s.title}</div>
                                <div class="stream-desc">${s.description}</div>${s.video_url ? `<video controls src="${s.video_url}"></video>` : '<p style="color: #666; font-style: italic;">No video feed attached.</p>'}
                            </div>
                        `).join('') : '<p style="color: #666; text-align: center; padding: 40px;">No media streams currently active on the island.</p>'}
                    </div>
                </div>
                <div class="footer">
                    Anadolu Island Sovereign Network &bull; Secured by SQLite & Express
                </div>
            </body>
            </html>
            `;
...
// Server Listen
app.listen(PORT, () => {
    console.log(`🚀 Sovereign Engine successfully online on port ${PORT}`);
});
